...
function peer_cert() {
    ...
    mkdir -p "organizations/peerOrganizations/$ORG.ac.uk/ca"
    mkdir -p "organizations/peerOrganizations/$ORG.ac.uk/msp/cacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.ac.uk/msp/tlscacerts"
    mkdir -p "organizations/peerOrganizations/$ORG.ac.uk/peers"
    mkdir -p "organizations/peerOrganizations/$ORG.ac.uk/tlsca"

    CERT_DIR=organizations/peerOrganizations/$ORG.ac.uk

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-peer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.$ORG.ac.uk-cert.pem"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/ca/ca.$ORG.ac.uk-cert.pem"
...
function orderer_cert() {
    TYPE=$1 #orderer user
    USER=$2 #orderer.ac.uk

    mkdir -p organizations/ordererOrganizations/ac.uk/ca
    mkdir -p organizations/ordererOrganizations/ac.uk/msp/cacerts
    mkdir -p organizations/ordererOrganizations/ac.uk/msp/tlscacerts
    mkdir -p organizations/ordererOrganizations/ac.uk/orderers
    mkdir -p organizations/ordererOrganizations/ac.uk/tlsca

    CERT_DIR=organizations/ordererOrganizations/ac.uk

    if [ ! -f "$CERT_DIR/ca/ca-key.pem" ]; then

        cfssl gencert -initca "${PWD}/organizations/cfssl/ca-orderer.json" | cfssljson -bare "$CERT_DIR/ca/ca"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/tlsca/tlsca.ac.uk-cert.pem"

        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/cacerts/"
        cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/msp/tlscacerts/tlsca.ac.uk-cert.pem"
...
function generate_orderer_certs() {
    ...
    for DIR in cacerts keystore signcerts tlscacerts; do
        mkdir -p "organizations/ordererOrganizations/ac.uk/orderers/$USER/msp/$DIR"
    done

    mkdir -p "organizations/ordererOrganizations/ac.uk/orderers/$USER/tls"
    ...
    cp "$CERT_DIR/ca/ca.pem" "$CERT_DIR/orderers/$USER/msp/tlscacerts/tlsca.ac.uk-cert.pem"
